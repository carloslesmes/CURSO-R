...
El paquete `ggplot2` es un paquete que hace gráficas a partir de los datos. Primero, instálelo con `install.packages("ggplot2")` y activelo con `library(ggplot2)`. Los paquetes `ggExtra` y `ggthemes` son paquetes adicionales que ayudan a mejorar sus gráficas, de igual forma se instalan y se activan. Para saber más vea @Rman y @HWggplot.
...
```
Si quiere guardar la gráfica, la función `ggsave()` guarda la última gráfica realizada, por ejemplo :
...
citation("ggExtra")
```
Para más vea [galleria](https://r-graph-gallery.com)